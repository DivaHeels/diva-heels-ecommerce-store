...
import { getProductMinorPrice, isCountryAllowed, isMarket, markets } from '@/lib/markets'
...
export async function POST(request: Request) {
  try {
    ...
    if (requested.some((item: any) => !item.slug || !item.size || !Number.isInteger(item.quantity) || item.quantity < 1 || item.quantity > 20) || requested.reduce((sum: number, item: any) => sum + item.quantity, 0) > 50) {
    ...
    if (requested.some((item: any) => !bySlug.has(item.slug))) return NextResponse.json({ error: 'Един от продуктите вече не е наличен.' }, { status: 409 })
