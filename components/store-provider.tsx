...
function CartDrawer() {
  ...
  const { t } = useLanguage()
  return open ? <div className="fixed inset-0 z-50 bg-black/25" onClick={() => setOpen(false)}><aside className="absolute right-0 top-0 h-full w-full max-w-md bg-white p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}><div className="flex items-center justify-between border-b border-[#e8e5e1] pb-5"><h2 className="font-editorial text-3xl">{t('cart')}</h2><button aria-label={t('close')} onClick={() => setOpen(false)} className="text-2xl">×</button></div>{items.length === 0 ? <p className="py-12 text-center text-sm text-[#666]">{t('emptyCart')}</p> : <><div className="divide-y divide-[#e8e5e1]">{items.map((i) => <div className="flex gap-4 py-5" key={i.product.slug + i.size}><img src={i.product.images[0]} alt={i.product.title} className="h-24 w-20 object-cover" /><div className="flex-1"><div className="flex justify-between gap-2"><p className="text-sm">{i.product.title}</p><button onClick={() => remove(i.product.slug, i.size)} className="text-xs underline">{t('remove')}</button></div><p className="mt-1 text-xs text-[#666]">{t('size')} {i.size}</p><div className="mt-4 flex items-center justify-between"><div className="flex items-center border border-[#e8e5e1]"><button className="px-3 py-1" onClick={() => update(i.product.slug, i.size, Math.max(1, i.quantity - 1))}>−</button><span className="px-2 text-sm">{i.quantity}</span><button className="px-3 py-1" onClick={() => update(i.product.slug, i.size, i.quantity + 1)}>+</button></div><b className="text-sm">{formatPrice(i.product)}</b></div></div></div>)}</div><div className="mt-auto border-t border-[#e8e5e1] pt-5"><div className="flex justify-between text-sm"><span>{t('subtotal')}</span><b>{formatMinor(total, market)}</b></div><p className="mt-2 text-xs text-[#666]">{t('shipping')}: {t('free')}</p><a href="/checkout" className="mt-5 block bg-[#111] py-4 text-center text-sm text-white">{t('checkout')}</a></div></>}</aside></div> : null
}
