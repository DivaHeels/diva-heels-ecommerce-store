...
export async function createRevolutOrder(amountMinor: number, currency: string, reference: string): Promise<RevolutOrder | null> {
  ...
  const response = await fetch(`${baseUrl}/api/orders`, { method: 'POST', headers: { Authorization: `Bearer ${secret}`, 'Content-Type': 'application/json', 'Revolut-Api-Version': API_VERSION }, body: JSON.stringify({ amount: amountMinor, currency, merchant_order_data: { reference } }), cache: 'no-store' })
